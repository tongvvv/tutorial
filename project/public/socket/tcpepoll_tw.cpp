...
int main(int argc,char *argv[])
{
  ...
  ev.data.fd=listensock; //事件的自定义数据，会随着epoll_wait()返回的事件一并返回

  //将listen的事件加入epollfd句柄.
  ...
  while (true)
  {
	...
	epoll_ctl(epollfd,EPOLL_CTL_ADD,clientsock,&ev);       
      ...
      else
      {
        ...
        if (recv(evs[ii].data.fd,buffer,1000,0)<=0)
        {
          // 如果客户端的连接已断开。
          printf("client(eventfd=%d) disconnected.\n",evs[ii].data.fd);
          close(evs[ii].data.fd);            // 关闭客户端的socket
	  // 如果关闭了socket,epoll自动处理epollfd，不需要调用epoll_ctl()删除这个socket
        }
        else
        {
          // 如果客户端有报文发过来。
