//poll select本质上没有区别，poll放弃了bitmap，采用数组表示法
//每次调用poll，要把数组从用户态拷贝到内核，之后又要拷贝回来
//poll返回之后也需要遍历数组，效率比较低
//select监听可写事件时，只要socket可写，就立刻返回
//而poll监听可写事件时，即使socket可写，也不一定返回
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
#include <string.h>
...
int main(int argc,char *argv[])
{
  ...
  while (true)
  {
    ...
    for (int eventfd=0;eventfd<=maxfd;eventfd++)
    {
      ...
      //如果有事件发生，那么revents&相应的事件类型，得到的结果不等于0
      ...
      if (eventfd==listensock)
      {
        ...
        fds[clientsock].events=POLLIN|POLLOUT;
      ...
      fds[eventfd].revents=0;  // 把revents清空。
    }
  }
