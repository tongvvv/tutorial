 ...
 *错误代码
  ...
  oracle不需要为查询语句新建连接
  oracle不需要MAXPARAMS宏
  如果sql语句有问题oracle的prepare不会返回错误，mysql的prepare会返回错误，尽可能不判断prepare的返回值
 * */
#include"_public.h"
#include"_ooci.h"
#define MAXPARAMS 256  //结果集字段的最大数量
int imaxfieldlen=-1;

char strfieldname[MAXPARAMS][31]; //结果集字段名的数组，从starg.fieldstr解析得到
...
{ 
  ...
  printf("Example:/project/tools1/bin/procctl 3600 /project/tools1/bin/dminingoracle /log/idc/dminingoracle_ZHOBTCODE.log \"<connstr>qxidc/qxidcpwd@tw_oracle</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><selectsql>select obtid,cityname,provname,lat,lon,height from T_ZHOBTCODE</selectsql><fieldstr> obtid,cityname,provname,lat,lon,height</fieldstr><fieldlen>10,30,30,10,10,10</fieldlen><bfilename>ZHOBTCODE</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata1</outpath><timeout>10</timeout><pname>dminingoracle_ZHOBTCODE</pname>\" \n\n"); 

  printf("Example:/project/tools1/bin/procctl 30 /project/tools1/bin/dminingoracle /log/idc/dminingoracle_ZHOBTMIND.log \"<connstr>qxidc/qxidcpwd@tw_oracle</connstr><charset>Simplified Chinese_China.AL32UTF8</charset><selectsql>select obtid,to_char(ddatetime,'yyyymmddhh24miss'),t,p,u,wd,wf,r,vis,keyid from T_ZHOBTMIND where keyid>:1 and ddatetime>sysdate-0.03 </selectsql><fieldstr>obtid,ddatetime,t,p,u,wd,wf,r,vis,keyid</fieldstr><fieldlen>10,19,8,8,8,8,8,8,8,15</fieldlen><bfilename>ZHOBTMIND</bfilename><efilename>HYCZ</efilename><outpath>/idcdata/dmindata1</outpath><starttime></starttime><incfield>keyid</incfield><incfilename>/idcdata/dmining/dminingoracle_ZHOBTMIND_HYCZ.list</incfilename><timeout>30</timeout><pname>dminingoracle_ZHOBTMIND_HYCZ</pname><maxcount>1000</maxcount><connstr1>qxidc/qxidcpwd@tw_oracle</connstr1>\" \n\n"); 
  ...
  printf("pname       进程名，建议用\"dminingoracle_后缀\"的方式\n\n");
}
...
bool _xmltoarg(char* strxmlbuffer)
{
  ...
  for(int ii=0 ; ii<CmdStr.CmdCount(); ii++)
  {
    ...
    //if(ifieldlen[ii]>imaxfieldlen) { ifieldlen[ii]=imaxfieldlen ;} //字段长度不能超过imaxfieldlen
    if(ifieldlen[ii]>imaxfieldlen) { imaxfieldlen=ifieldlen[ii]; }  //得到最大字段值的长度
  }
