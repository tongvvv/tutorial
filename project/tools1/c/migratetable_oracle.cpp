...
void _help(char *argv[])
{
  ...
  printf("Sample:/project/tools1/bin/procctl 3600 /project/tools1/bin/migratetable_oracle /log/idc/migratetable_oracle_ZHOBTMIND.log \"<connstr>qxidc/qxidcpwd@tw_oracle</connstr><srctname>T_ZHOBTMIND</srctname><dsttname>T_ZHOBTMIND_HIS</dsttname><keycol>keyid</keycol><where> where ddatetime<sysdate-0.03 </where><starttime>01,02,03,04,05,13</starttime><maxcount>300</maxcount><timeout>120</timeout><pname>migratetable_ZHOBTMIND</pname>\"\n\n");

  printf("本程序是数据中心的公共功能模块，用于定时迁移表中的数据。\n");
  ...
  printf("dsttname    迁移目的表的表名，注意，srctname和dsttname结构必须完全相同。\n");
...
bool _migratetable()
{
  ...
  //拼接绑定删除sql语句where 唯一键 in (%s) 的字符串	
  char bindstr[2001] = {0};
  char strtemp[11];
  ...
  sqlstatement stmtdel(&conn1); //执行删除数据的sql语句
  ...
  if(stmtsel.m_cda.rpc > 0 ) { logfile.Write("migrate %s to %s %d rows in %.2fsec.\n",starg.srctname,starg.dsttname,stmtsel.m_cda.rpc,Timer.Elapsed());}
