...
void _help()
{
  ...
  printf("Example:/project/tools1/bin/procctl 20 /project/tools1/bin/tcpputfiles /tmp/tcpputfiles.log \"<ip>101.34.83.118</ip><port>5005</port><ptype>1</ptype><clientpath>/tmp/tcp/surfdata1</clientpath><clientpathbak>/tmp/tcp/surfdata1bak</clientpathbak><andchild>true</andchild><matchname>*.xml,*.csv</matchname><srvpath>/tmp/tcp/surfdata2</srvpath><timevl>10</timevl><timeout>50</timeout><pname>tcpputfiles_surfdata</pname>\" \n\n");
  ...
  printf("<ip>101.34.83.118</ip> 服务端ip\n");
...
bool _tcpputfiles()
{
  ...
  { 
    ...
    PActive.UptATime();
    //接收对端确认报文
    //这里TcpRead函数不等待，如果对端还没来得及发送确认报文，直接break，继续传文件。
    while(delayed>0)
    {
      memset(strrecvbuffer,0,sizeof(strrecvbuffer));
      if(TcpRead(TcpClient.m_connfd,strrecvbuffer,&ibuflen,-1) == false)
      {
 	break;
      }
      delayed--;
  ...
  {
    memset(strrecvbuffer,0,sizeof(strrecvbuffer)); 
    if(TcpRead(TcpClient.m_connfd,strrecvbuffer,&ibuflen,10) == false)
    {
      break;
    }
    delayed--;
