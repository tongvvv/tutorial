...
struct st_arg
{
  ...
  char lnktname[31];       // 远程表名，在remotetname后面加@dblink。
...
int main(int argc,char *argv[])
{
  ...
  //如果remotecols或localcols为空，那么用starg.localtname表的全部列来填充
  ...
  while(true)
  {
    if(_syncincrement_oracle(&bcontinue)==false) return -1;
...
void _help(char *argv[])
{
  ...
  printf("       /project/tools1/bin/procctl 10 /project/tools1/bin/syncincrement_oracle /log/idc/syncincrement_oracle_ZHOBTMIND2.log \"<localconnstr>scott/tiger@tw_oracle</localconnstr><charset>Simplified Chinese_China.AL32UTF8</charset><lnktname>T_ZHOBTMIND1@db132</lnktname><localtname>T_ZHOBTMIND2</localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid</localcols><remoteconnstr>qxidc/qxidcpwd@tw_oracle</remoteconnstr><remotetname>T_ZHOBTMIND1</remotetname><remotekeycol>keyid</remotekeycol><localkeycol>recid</localkeycol><maxcount>300</maxcount><timevl>2</timevl><timeout>50</timeout><pname>syncincrement_oracle_ZHOBTMIND2</pname>\"\n\n");

  printf("       /project/tools1/bin/procctl 10 /project/tools1/bin/syncincrement_oracle /log/idc/syncincrement_oracle_ZHOBTMIND3.log \"<localconnstr>scott/tiger@tw_oracle</localconnstr><charset>Simplified Chinese_China.AL32UTF8</charset><lnktname>T_ZHOBTMIND1@db132</lnktname><localtname>T_ZHOBTMIND3</localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid</localcols><where> and obtid like '54%%' </where><remoteconnstr>qxidc/qxidcpwd@tw_oracle</remoteconnstr><remotetname>T_ZHOBTMIND1</remotetname><remotekeycol>keyid</remotekeycol><localkeycol>recid</localkeycol><maxcount>300</maxcount><timevl>2</timevl><timeout>50</timeout><pname>syncincrement_oracle_ZHOBTMIND3</pname>\"\n\n");
  ...
  printf("lnktname      远程表名，在remotetname后面加上@dblink。\n");
  ...
  printf("maxcount      每批执行一次同步操作的记录数，不能超过MAXPARAMS宏（在_mysql.h中定义）。\n");
  ...
  *bcontinue = false;
  ...
  while(true)
  {
    ...
    if(ccount == starg.maxcount)
    {
       ...
       ccount=0; memset(keyvalues,0,sizeof(keyvalues));
    }
    PActive.UptATime();
  }
  
  //bindstr内字符串从 :1到:starg.maxcount,如果剩下的一批key值不够这么多，那么后面有部分位置会置为空，语句正常执行；
  //剩下的不够一批
  if(ccount>0)
  {
     if(stmtins.execute() != 0)
