...
#define MAXCOLCOUNT 300    //每个表字段的最大数量
#define MAXCOLLEN   100    //表字段值的最大长度
sqlstatement  stmtins,stmtupt;
char strcolvalue[MAXCOLCOUNT][MAXCOLLEN+1] = {0}; //存放从xml文件每一行解析出来的值
void preparesql();
...
int _xmltodb(char* fullfilename,char* filename)
{
  ...
  while(true)
  {
    ...
    {
      if(stmtins.m_cda.rc == 1062) //记录已存在
      {
          if(stxmltotable.uptbz == 1) 
          {
             if( stmtupt.execute() != 0 )
             {
               ...
               //数据库连接已失效，无法继续，只能返回
               //1053-在操作过程中服务器关闭，2013-在查询过程中丢失与mysql服务器的连接
...
void crtsql()
{
  ...
  for(int ii=0 ;ii<TABCOLS.m_vallcols.size(); ii++)
  {
    ...
    if(strcmp(TABCOLS.m_vallcols[ii].datatype,"date") != 0) 
      ...
      { SNPRINTF(strtemp,sizeof(strtemp),sizeof(strtemp)-1,"str_to_date(:%d,'%%%%Y%%%%m%%%%d%%%%H%%%%i%%%%s')",colseq++);}
  ...
  for(int jj=0; jj<TABCOLS.m_vallcols.size(); jj++)
  {
     ...
     if(strcmp(TABCOLS.m_vallcols[jj].datatype,"date") != 0)
      ...
      { SNPRINTF(strtemp,sizeof(strtemp),sizeof(strtemp)-1,"%s=str_to_date(:%d,'%%%%Y%%%%m%%%%d%%%%H%%%%i%%%%s')",TABCOLS.m_vallcols[jj].colname,colseq++);}
  ...
  for(int jj=0; jj<TABCOLS.m_vallcols.size(); jj++)
  {
     ...
     if(strcmp(TABCOLS.m_vallcols[jj].datatype,"date") != 0)
      ...
      { SNPRINTF(strtemp,sizeof(strtemp),sizeof(strtemp)-1," and %s=str_to_date(:%d,'%%%%Y%%%%m%%%%d%%%%H%%%%i%%%%s')",TABCOLS.m_vallcols[jj].colname,colseq++);}
