...
void _help(char *argv[])
{
  ...
  printf("Sample:/project/tools1/bin/procctl 3600 /project/tools1/bin/deletetable /log/idc/deletetable_ZHOBTMIND1.log \"<connstr>101.34.83.118,root,admin,mysql,3306</connstr><tname>T_ZHOBTMIND1</tname><keycol>keyid</keycol><where>where ddatetime<timestampadd(minute,-120,now())</where><starttime>01,02,03,04,05,13</starttime><timeout>120</timeout><pname>deletetable_ZHOBTMIND1</pname>\"\n\n");
...
bool _deletetable()
{
  ...
  //准备删除本地表数据的sql语句
  sqlstatement stmtdel(&conn2); //执行删除数据的sql语句
