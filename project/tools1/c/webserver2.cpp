...
#define MAXCONNS 10              //数据库连接池大小
connection conns[MAXCONNS];      //数据库连接池
pthread_mutex_t mutex[MAXCONNS]; //池子里每个数据库连接的锁
bool initconns();      		 //初始化数据库连接池
connection* getconns();          //从数据库连接池中获得一个空闲的连接
bool freeconns(connection* conn);//释放/归还数据库连接
bool destoryconns();             //释放数据库连接占用的资源,(断开连接，销毁锁)
...
bool freeconns(connection* conn)
{
   for(int ii=0; ii<MAXCONNS; ii++)
   {
     if(&conns[ii] == conn)
     {
        pthread_mutex_unlock(&mutex[ii]); conn=0; return true;
     }
   } 
   return false;
}

//释放数据库连接占用的资源,(断开连接，销毁锁)
bool destoryconns()
{
  for(int ii=0; ii<MAXCONNS; ii++)
  {
