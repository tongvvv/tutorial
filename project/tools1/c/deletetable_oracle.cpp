...
int main(int argc,char *argv[])
{
  ...
  if (conn1.connecttodb(starg.connstr,NULL,1) != 0)
  {
...
void _help(char *argv[])
{
  ...
  printf("Sample:/project/tools1/bin/procctl 3600 /project/tools1/bin/deletetable_oracle /log/idc/deletetable_oracle_ZHOBTMIND1.log \"<connstr>qxidc/qxidcpwd@tw_oracle</connstr><tname>T_ZHOBTMIND1</tname><keycol>keyid</keycol><where>where ddatetime<sysdate-0.03</where><starttime>01,02,03,04,05,13</starttime><timeout>120</timeout><pname>deletetable_ZHOBTMIND1</pname>\"\n\n");
...
bool _deletetable()
{
  ...
  char tmpvalue[51];  //存放从表中提取的待删除记录的唯一键的值
  ...
  char keyvalues[MAXPARAMS][51]; //存放key字段的值
  ...
  if(stmtsel.m_cda.rpc > 0 ) { logfile.Write("delete from %s %d rows in %.2fsec.\n",starg.tname,stmtsel.m_cda.rpc,Timer.Elapsed());}
