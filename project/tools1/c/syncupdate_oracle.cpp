...
void _help(char *argv[])
{
  ...
  printf("where         同步数据的条件，填充在delete本地表和select fed表sql语句中，本参数可以为空，表示同步全部的记录。\n"\
                        "注意: 1)where中的字段必须同时存在于本地表和fed表中，2)不要用系统时间作为条件.\n");
...
bool _syncupdate_oracle()
{
  ...
  //如果不分批同步，表示需要同步的数据量比较少，执行一次sql语句就可以确定
  if(starg.synctype == 1)
  {
   ...
   if(stmtins.execute() != 0)
    {
      logfile.WriteEx("failed.\n");
      logfile.Write("stmtdel.execute() failed.\n%s\n%s\n",stmtdel.m_sql,stmtdel.m_cda.message);
      connloc.rollback(); //这里不必手动回滚，connection类的析构函数会自动回滚事务； 
      return false;
    } 
   
    logfile.WriteEx("ok. %d rows in %.2fsec.\n",stmtins.m_cda.rpc,Timer.Elapsed());
